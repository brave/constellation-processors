...
async fn main() {
  ...
  let gen_tasks: Vec<JoinHandle<Vec<String>>> = (0..cli_args.unique_count).map(|i| {
    ...
    tokio::spawn(async move {
      ...
      let measurement_layers: Vec<_> = (0..cli_args.layer_count).map(|i| {
        let r: u32 = rng.gen();
        format!("layer{}|{}", i, r).as_bytes().to_vec()
      }).collect();
      let example_aux = vec![];

      let mut curr_tag = None;

      (0..cli_args.threshold).map(|_| {
        ...
        let msg = client::construct_message(mgf, &example_aux, cli_args.threshold)
          .unwrap();

        let mbytes = base64::decode(&msg).unwrap();
        let snm: SerializableNestedMessage = bincode::deserialize(&mbytes).unwrap();
        let nm = NestedMessage::from(snm);
        if let Some(curr_tag) = &curr_tag {
          if &nm.unencrypted_layer.tag != curr_tag {
            println!("Diff tag!");
          }
        } else {
          curr_tag = Some(nm.unencrypted_layer.tag.clone());
        }
  ...
  let message_chunks: Vec<Vec<String>> = messages.chunks(message_count / cli_args.task_count)
    .map(|v| v.to_vec())
    .collect();
  ...
  let tasks: Vec<_> = message_chunks.into_iter().map(|chunk| {
  ...
  println!("Sent {} messages, took {} seconds, rate {} reqs/s",
